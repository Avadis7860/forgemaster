...
def _seed_committed_feature(conn, settings, git, *, red_finding=False, stale=False):
    """Projet + feature + task in_progress, worktree réservé avec un commit worker, verdict Tier-1 écrit
    (propre, ou 🔴, ou périmé). Retourne (sot, head_sha)."""
    ...
    findings = ([{"severity": "🔴", "file": "core.py", "line": 1, "evidence": "core.py:1 — value = 1"}]
                if red_finding else [])
