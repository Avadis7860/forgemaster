...
def test_browser_game_wires_game_dev_identity():
    ...
    # (a') NEUTRALITÉ : le générique n'impose aucun genre — pas d'ogame, pas de « PvE vs bots ».
    assert "starter générique neutre" in claude
    for forbidden in ("OGame-like", "OGame-like / roguelike", "PvE vs bots"):
        assert forbidden not in claude, f"générique browser-game pollué par un genre : « {forbidden} »"
