...
// le cockpit depuis une AUTRE machine (le navigateur résout `127.0.0.1` sur la machine du viewer, pas sur
// l'hôte). Le host joignable d'un produit, depuis le navigateur, est celui par lequel on a atteint le
// cockpit — exactement le raisonnement de `ws.ts` (`window.location.host`). On recompose donc le lien à
