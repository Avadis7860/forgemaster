...
Le cockpit est une **spine CLI + cœur déterministe**, avec un **daemon FastAPI** et un **web** comme vues
...
```
   ...
   CLI `cockpit`   ───▶│           découpés par domaine)  ·  terminal/  │
                       └───────────────────────────────────────────────┘
                                          │  (délègue au cœur)
        ┌─────────────┬───────────────────┼───────────────────┬─────────────┐
        ▼             ▼                    ▼                   ▼             ▼
...
- **`config`** — le socle : résolveur générique des racines (`COCKPIT_HOME`, `COCKPIT_PROJECTS_ROOT`),
...
## Frontières délibérées — ce que cockpit n'est PAS
...
- **Pas de dépendance BWS obligatoire.** Un secret de push est une **référence** résolue en amont par
  l'appelant ; le cockpit ne connaît pas BWS (spec merge-writeback).
- **Pas une forge externe (Forgejo/GitLab).** Le cockpit **EST** la forge (native-IA-worker) ; GitHub est
