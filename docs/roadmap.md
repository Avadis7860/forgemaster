# Roadmap — forgemaster (produit)

> Roadmap **produit durable** du forgemaster : la vision, ce qui est **livré** en V1, les **décisions de
...
Faire du forgemaster un **outil distribuable, self-hosted, professionnel** : chaque projet créé naît
**auto-travaillable** (3 index déterministes isolés — code / docs / front — + un `claude` dispatchable),
le forgemaster distingue **projets** et **outils**, donne de la **visibilité git**, et **onboarde ses tokens**
au 1er démarrage (dont un token par repo) — **sans jamais** de secret en clair ni de pollution croisée
entre repos.

Modèle produit assumé : **cœur léger + extensions**. Tout ce qui est natif forgemaster et auto-intégré à
chaque projet (code-map, docs-map, front-map, forgemaster-catalogs) est un **outil** (`kind=tool`) ; le forgemaster
...
| **P5** | Distribution turnkey : wheel Node-less (dist embarquée), `forgemaster setup` build le front from-clone, dist absente → **page fail-loud actionnable** (jamais un 404 muet), garde-fou comportemental « le wheel SERT la SPA à `/` » | ✅ | `deploy/acceptance-fresh-venv.sh` `[4/4]` |

### Détail P4 — le store de secrets (self-hosted)

- **Module interne** `src/forgemaster/secrets/` derrière un Protocol `SecretStore` (`put→ref` / `get(ref)` /
...
- **Défaut = `EncryptedFileStore`** : chiffrement authentifié au repos (Fernet, `cryptography` cœur),
  ...
  `credential_ref`, extra `forgemaster[bws]`). Keyring OS différé (fragile headless). Store choisi
  **globalement par instance** via `FORGEMASTER_SECRET_STORE` (défaut `file`) ; `credential_ref` **opaque**.
- **git n'importe JAMAIS `forgemaster.secrets`** : le writeback reçoit un `credential_ref`, résolu **lazy+total**
...
- Surface : CLI `forgemaster onboard status|link|unlink` + routes `GET /api/onboarding`,
...
- **Claude = par-environnement, pas par-projet** — un seul binaire par CT/env sert tous les projets ;
  l'utilisateur lie son compte lui-même (login interactif). Aucun secret claude injecté par le forgemaster.
...
- **Ne jamais committer `web/dist`** — la dist voyage dans le wheel (build mainteneur, Node) ; la
  provision cible est Python-seul (spec [`web-forgemaster-spa`](specs/web-forgemaster-spa.md)). `forgemaster serve`
  n'auto-build **jamais** : build (mainteneur) et provision (cible) restent séparés.
- **Maison du capital visuel = vendored/forgemaster, MCP différé** — les templates UI de référence vivent
  côté forgemaster (`web/dist/templates/`), pas dans un index MCP `ui-kit`. Données terrain : N=1 template,
...
- **Tag des 5 repos framework en `tool`** — le mécanisme (`--kind tool`) est livré ; le tag effectif se
  fait à l'adoption de ces repos comme entités. Un `kind=core` distinct du forgemaster reste à trancher.
