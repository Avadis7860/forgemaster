...
`src/forgemaster/provision/__init__.py:79` · appelé par `projects.registry.create_project` (via la policy), `read_bundle_manifest`, `validate_bundle`, `list_valid_types`, `load_launch_roadmap`, `load_payload`, `manage._show`.
...
`src/forgemaster/provision/__init__.py:122` · appelé par `projects.registry.create_project`, `list_valid_types`, `manage._list/_validate/_show`.
...
`src/forgemaster/provision/__init__.py:176` · appelé par le dropdown de création (UI) et le durcissement des `choices` CLI.
...
`src/forgemaster/provision/__init__.py:105` · appelé par `list_valid_types`, `manage._list/_show/_version`.
...
`src/forgemaster/provision/__init__.py:202` · appelé au seed d'un projet (create).
...
`src/forgemaster/provision/__init__.py:249` · appelé par les appelants historiques non-typés.
...
`src/forgemaster/provision/facet.py:64` · appelé au dispatch (canal hooks+permissions).
...
`src/forgemaster/provision/mcp.py:200` · appelé par `cli_dispatch` (`forgemaster mcp wire`) **et** par la route onboarding du wizard.
...
`src/forgemaster/provision/mcp.py:61` · appelé par `inject_mcp_config`.
...
`src/forgemaster/provision/mcp.py:84` · appelé au dispatch d'un worker (injection POST-création).
...
`src/forgemaster/provision/mcp.py:129` · appelé par `check_lifecycle`.
...
`src/forgemaster/provision/mcp.py:115` · appelé par `check_lifecycle`.
...
`src/forgemaster/provision/mcp.py:143` · appelé par `forgemaster doctor`.
...
`src/forgemaster/provision/mcp.py:195` (`ValueError`) · levée par `wire`, interceptée par `cli_dispatch` et la route onboarding.
...
`src/forgemaster/provision/mcp.py:184` · lu par `onboarding.status` (axe `mcp` du wizard).
