...
`src/cockpit/provision/__init__.py:79` · appelé par `projects.registry.create_project` (via la policy), `read_bundle_manifest`, `validate_bundle`, `list_valid_types`, `load_launch_roadmap`, `load_payload`, `manage._show`.
...
`src/cockpit/provision/__init__.py:202` · appelé au seed d'un projet (create).
...
`src/cockpit/provision/__init__.py:249` · appelé par les appelants historiques non-typés.
...
`src/cockpit/provision/__init__.py:33` · levée par `discover_types`-gated helpers, `load_bundle`, `_parse_manifest`, `validate_bundle`, `load_launch_roadmap`.
...
`src/cockpit/provision/facet.py:64` · appelé au dispatch (canal hooks+permissions).
