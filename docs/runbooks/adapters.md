...
`src/cockpit/terminal/registry.py:176` · instancié au lifespan du daemon (`daemon/app.py`, porté par `app.state.terminals`) et passé à `serve_project_terminal` par le router.
...
`src/cockpit/terminal/pty.py:68` · appelé par `_relay` (branche frame TEXTE du sens WS→PTY).
...
`src/cockpit/terminal/pty.py:60` · appelé par le router terminal pour poser le `cwd` du PTY.
