...
`src/forgemaster/cli.py:431` · point d'entrée console_scripts
...
Le cœur du dispatch est la **table `_HANDLERS`** (`cli.py:685`), un dict `command → _h_*` : chaque `_h_*`
...
`src/forgemaster/cli.py:406` (`_settings`) · `src/forgemaster/cli.py:416` (`_autoload_env`) · appelés par main()
...
- **La table `_HANDLERS`** (`cli.py:685`) : le mapping `command → _h_*`, décrit dans la section `main()`.
