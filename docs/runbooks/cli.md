...
`src/cockpit/cli.py:364` · point d'entrée console_scripts
...
Le cœur du dispatch est la **table `_HANDLERS`** (`cli.py:601`), un dict `command → _h_*` : chaque `_h_*`
...
`src/cockpit/cli.py:345` (`_settings`) · `src/cockpit/cli.py:350` (`_autoload_env`) · appelés par main()
...
- **La table `_HANDLERS`** (`cli.py:601`) : le mapping `command → _h_*`, décrit dans la section `main()`.
