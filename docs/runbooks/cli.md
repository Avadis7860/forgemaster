...
`src/forgemaster/cli.py:423` · point d'entrée console_scripts
...
Le cœur du dispatch est la **table `_HANDLERS`** (`cli.py:677`), un dict `command → _h_*` : chaque `_h_*`
...
`src/forgemaster/cli.py:398` (`_settings`) · `src/forgemaster/cli.py:408` (`_autoload_env`) · appelés par main()
...
- **La table `_HANDLERS`** (`cli.py:677`) : le mapping `command → _h_*`, décrit dans la section `main()`.
