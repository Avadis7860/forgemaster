...
`src/forgemaster/cli.py:383` · point d'entrée console_scripts
...
Le cœur du dispatch est la **table `_HANDLERS`** (`cli.py:628`), un dict `command → _h_*` : chaque `_h_*`
...
`src/forgemaster/cli.py:363` (`_settings`) · `src/forgemaster/cli.py:368` (`_autoload_env`) · appelés par main()
...
- **La table `_HANDLERS`** (`cli.py:628`) : le mapping `command → _h_*`, décrit dans la section `main()`.
- **Le détail des sous-parsers/arguments** de chaque sous-commande (flags, `choices`, `nargs`) : lire
  directement `build_parser` (`cli.py:22`–306) — c'est de la déclaration argposée, pas de la logique.
