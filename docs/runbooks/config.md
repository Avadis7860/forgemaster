...
`src/forgemaster/bootstrap.py:81` · appelé par `cli_dispatch` (`:144`), après `load_manifest` + `_resolve_shared_ref`
...
`src/forgemaster/bootstrap.py:51` · appelé par `preview` (`:110`) et `cli_dispatch` (`:144`)
...
`src/forgemaster/bootstrap.py:110` · surface lecture (wizard `/setup`), miroir sans effet de `run_bootstrap`
...
`src/forgemaster/bootstrap.py:124` · appelé par `cli_dispatch` sur `--init` (`:150`)
