...
Le schéma SQLite du cockpit est un **contrat figé** : le fichier `schema.py` porte l'unique définition, et toute
...
`src/cockpit/db/store.py:16` · appelé par `open_db()`
Crée le dossier parent, pose `row_factory = Row` (accès dict-like), `PRAGMA foreign_keys = ON`, `journal_mode = WAL`
et `busy_timeout = 5000`. Le WAL + busy_timeout servent la concurrence CLI↔daemon sous `cockpit run`
...
`src/cockpit/db/store.py:32` · appelé par `open_db()` · retourne la version finale
...
## open_db() — point d'entrée : connexion migrée à la base du cockpit
`src/cockpit/db/store.py:41` · appelé par les couches supérieures (registry, CRUD)
