...
`src/forgemaster/daemon/app.py:165` · appelé par `apply_update._wait_health` et par le front (`useHealth`, 10 s)
...
`src/forgemaster/daemon/app.py:322` (`serve`) · `:239` (`_mount_spa`) · `:26` (`web_dist_dir`)
...
- `_mount_missing_ui_placeholder` (`app.py:292`) : dist absente → warning + page d'aide fail-loud à `/`, l'API reste valable.
