...
`src/cockpit/dispatch/jobs.py:34` · `src/cockpit/dispatch/jobs.py:59` · appelés par `worker.dispatch_next`
...
`src/cockpit/dispatch/jobs.py:148` · appelé par `stream.stream_job` (boucle live) et `jobs.tail`
...
`src/cockpit/dispatch/jobs.py:172` · appelé par les lectures ponctuelles (historique / API non-live)
...
`src/cockpit/dispatch/jobs.py:135` · appelé par le suivi live pour localiser le fichier écrit par le worker
...
`src/cockpit/dispatch/jobs.py:235` · appelé par `read_events` sur chaque ligne complète
...
`src/cockpit/dispatch/reconcile.py:76` · appelé par la garde de finalisation de `worker.dispatch_next` et par `reconcile_orphans`
...
`src/cockpit/dispatch/worktree.py:88` · appelé par la santé/diagnostic du dispatch
...
`src/cockpit/dispatch/worktree.py:28` · appelé par `reserve`, `release`, `audit`
