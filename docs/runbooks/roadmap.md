...
`src/forgemaster/roadmap/model.py:81` · appelé par `add_task`, `resolver.index_for_feature`
...
`src/forgemaster/roadmap/model.py:96` · appelé par `cli_dispatch` (`roadmap set-deps`)
...
`src/forgemaster/roadmap/model.py:123` · appelé par `resolver.cli_dispatch` (`task add`), `seed.seed_launch_roadmap`
...
`src/forgemaster/roadmap/prompt.py:247` · appelé par la machinerie de dispatch (au lancement d'un worker sur la NEXT task)
...
`src/forgemaster/roadmap/seed.py:17` · appelé par `projects.registry.create_project` (chemin SEED d'un projet neuf uniquement)
Pure seed (aucune heuristique design-first ; l'ordre naît de la GRAINE `depends_on`, jamais du resolver). `load_launch_roadmap(project_type)` (fail-soft → 0) → boucle features (`model.add_feature`, `depends_on` inter-feature semable v10) puis tasks (`model.add_task`, `priority` défaut `P1`, **`mode` défaut `headless`**). Ce dernier compte : une graine peut poser `mode: interactive` sur une task de cadrage, et c'est ainsi que le besoin d'interview naît du **bundle** au lieu d'être décidé au dispatch (v12). Retourne le nombre de features semées. Idempotence : aucune — `add_feature`/`add_task` lèvent `ValueError` sur doublon, l'appelant enveloppe fail-soft (jamais de rollback SoT). Import paresseux côté registry (évite le cycle registry↔model).
...
- **resolver.py** : `_to_records`/`_feature_records` (projection rows forgemaster → forme record taskmap : `id≡slug`, `created≡created_at`, `tags:[]`, statut mappé), `_blockers`/`_feature_blockers` (re-traduction blockers en vocab forgemaster), `_classify_tm` (délégation `taskmap.classify`), `_counts` (tally d'états pour le rapport CLI), `cli_dispatch` (route `task add|set-deps|next`), constantes `PRIO`/`_STATUS_TO_TM`/`_FEATURE_STATUS_TO_TM`.
- **prompt.py** : `_mandate`, `_context_block`, `_facet_block`, `_acceptance_block`, `_design_block`, `_decisions_block` (helpers de composition détaillés inline dans `build_worker_prompt` ci-dessus), constantes `CONTEXT_DOCS`/`_EXCERPT_MAX`. Plus le couple de la passe de **fix** : `build_fix_prompt` (re-dispatch d'un worker sur un gate rouge) et `findings_block` (les findings du verdict, rendus en bloc — injectés aussi en preamble du minerai par `write_decision_doc`).
- **check.py** : `Issue` (dataclass frozen, énumération des `kind`), `cli_dispatch` (route `roadmap check`, rapport groupé par feature). Plus la garde de **profondeur** : `check_depth_axes` (une roadmap doit couvrir ses axes d'archétype, pas seulement compiler) et `load_deferred_axes` (les axes explicitement différés, qui ne comptent donc pas comme un trou).
