...
`src/forgemaster/update.py:136` · appelé par `preflight` et `preflight_rollback` · lève `UpdateRefused`
...
`src/forgemaster/update.py:177` · appelé par `preflight` et `preflight_rollback` · consomme `projects.authority.blocking`
...
`src/forgemaster/update.py:249` · appelé par `cli_dispatch` · retourne le plan (+ `snapshot`, `target_venv`) · lève `UpdateRefused`
...
`src/forgemaster/update.py:298` · appelé par `preflight_rollback` et `aptitude` · retourne `(cible, venv, motif)`
...
`src/forgemaster/update.py:336` · appelé par `preflight_rollback` et `_resolution_cible` · retourne `(venv cible, motif de refus)`
...
`src/forgemaster/update.py:381` · appelé par `_cible_utilisable` · retourne le venv ou `None`
...
`src/forgemaster/update.py:420` · appelé par `aptitude_route` (`src/forgemaster/daemon/routes/update.py:112`) et `_cli_aptitude` (`src/forgemaster/update.py:1189`) · **ne lève jamais**
...
**`OSError` est rattrapée, et ce n'est pas de la superstition.** `_preflight_service` lit l'unité (`up.read_text`)
**après** avoir vérifié qu'elle existe : une unité présente mais illisible — droits, contenu non-UTF-8 — remonte une
`PermissionError` / `UnicodeDecodeError`, **pas** un `UpdateRefused`. Trouvé en revue du diff puis **mesuré** :
sans ce rattrapage, `GET /aptitude` rendait **500** là où il promet 200, et « ne lève jamais » était une promesse plus
forte que le code. On ne rattrape **que** ce qu'on sait nommer — un `except Exception` avalerait les vraies pannes, ce
que ce module refuse partout ailleurs (`_restore` rend `False` plutôt que d'avaler). Les préflights, eux, gardent leur
exposition d'origine : ce défaut leur est **antérieur** et le corriger là-bas serait une autre PR.
...
sur événement (un run qui atteint son verdict), seul moment où le produit lui-même a pu déplacer la réponse. Son
`refetch` est stable (lié dans le constructeur de `QueryObserver`, vérifié dans `query-core`, pas supposé), sans quoi
l'effet qui le rappelle boucherait.

## parse_exec_start() — l'unité est la SEULE vérité sur le bind du service
`src/forgemaster/update.py:494` · appelé par `_preflight_service` · retourne `(binaire, host, port)` · lève `UpdateRefused`
...
`src/forgemaster/update.py:515` · `src/forgemaster/update.py:556` · appelés par `cli_dispatch` · seul contenu de `--dry-run`
...
`src/forgemaster/update.py:195` · appelé par `preflight` et `preflight_rollback` · consomme `dispatch.jobs.running`
...
`src/forgemaster/update.py:218` · appelé par `preflight` et `preflight_rollback` · consomme `survey_updates_in_flight`
...
`src/forgemaster/update.py:577` · appelé par `launch` et par `routes/update._lance` · rend `{run, unit, ok, detail}`
...
`src/forgemaster/update.py:656` · appelé par `cli_dispatch` · retourne le rc de `follow` (ou 0 si `--detach`)
...
`src/forgemaster/update.py:1033` · appelé par `launch` · rend le préfixe `systemd-run` de la commande
...
`src/forgemaster/update.py:733` · appelés par `routes/update` · lecture pure, aucun effet
...
`src/forgemaster/update.py:845` · appelés par `routes/update` et `cli_dispatch` · écriture bornée, purge à l'écriture
...
`forgemaster update wheels` (`_cli_wheels`, `src/forgemaster/update.py:1216`) — parce qu'une politique de rétention qui
...
`src/forgemaster/update.py:682` · appelé par `launch` · retourne le rc lu dans `result.json`
...
`src/forgemaster/update.py:1068` · appelés par `cli_dispatch` et par `routes/update._plan` · retournent les verdicts, ou `[]`
...
`src/forgemaster/update.py:1155` · appelé par `cli._h_update` (routé par `_HANDLERS`) · retourne le code de sortie
...
`src/forgemaster/apply_update.py:100` · appelé par `apply` · retourne le chemin du `forgemaster` neuf · lève `UpdateFailed`
Étapes [1/7] et [2/7]. À côté parce qu'un processus ne remplace pas le wheel qu'il exécute, et surtout parce que
...
`src/forgemaster/apply_update.py:115` · appelé par `apply` (étape 3/7) et `rollback` (étape 1/6) · retourne l'identité de build
...
`src/forgemaster/apply_update.py:356` · appelé par `probe_isolated` et `_verify_live` · retourne `(état, phrase)`
...
`src/forgemaster/apply_update.py:158` · appelé par `apply` (4/7) et `rollback` (2/6) · retourne le dossier · lève `UpdateFailed`
...
`src/forgemaster/apply_update.py:182` · appelé par `apply`, `rollback` · aucun retour
...
`src/forgemaster/apply_update.py:192` · appelé par `_verify_live` · **fonction pure** · retourne `(verdict, motif)`
...
## repose_maps() / edition_posable() — une édition monte ENTIÈRE, et redescend entière
`src/forgemaster/apply_update.py:431` (`edition_posable`) · `src/forgemaster/apply_update.py:431` (`repose_maps`) ·
appelés par `apply` (étape 6/7 et le retour automatique) et `rollback` (étape 5/6) · retournent `(état, dit)`
Une édition est un jeu de SHA épinglés : le wheel ET les 3 cartes hôte. Tant que la bascule ne déplaçait que le wheel,
« deux installs de la même édition posent le même code » n'était vrai qu'à l'**install fraîche** — et un retour qui ne
reposait pas les cartes produisait un état que personne n'a jamais eu (vieux binaire, cartes neuves), pire que celui
qu'on corrige. **C'est le venv qui pose SES cartes**, par sa propre commande (`toolchain install --maps-only`) : rien
n'est composé de l'extérieur, `tools.edition_maps_dir()` résout `_maps` dans CE paquet-là, donc l'aller pose l'édition
du bleu et le retour celle de la cible — même code, deux sens, aucune logique de retour à part. **Un subprocess, pas un
import** : ce module est stdlib-pur par contrat. La **frontière zéro-réseau** est tenue par l'étroitesse du geste
(`--maps-only` ne retient que l'étape `--no-index` du plan), pas par une intention.
Trois issues, vocabulaire repris de `check_ui` : `POSEES` · `NON_MESURE` (cible sans `_maps`, ou antérieure au drapeau,
ou venv muet — **annoncé, jamais bloquant** : *on exige la preuve pour avancer, pas pour revenir*) · `RATE` (pose
tentée, rc≠0 — **annoncé, pas fatal** : les cartes vivent dans un venv PARTAGÉ hors de la bascule, revenir en arrière
ne le répare pas et défait une instance qui sert). Seul `RATE` déplace l'`impact` : une pose qu'on n'a pas pu tenter
laisse le périmètre là où il était avant ce câblage. La capacité d'une cible se lit **dans le texte** de son `cli.py`
(même technique que `_supports_flag`), jamais déduite d'un numéro de version.

## annonce_maps_wheel() / annonce_maps_venv() — dit AVANT le geste
`src/forgemaster/apply_update.py:465` (`annonce_maps_wheel`) · `src/forgemaster/apply_update.py:421`
(`annonce_maps_venv`) · appelés par `update.preflight` / `preflight_rollback` · retournent une ligne de `describe`
Même levier qu'`annonce_verification` : le plan porte la ligne, `describe` la rend, donc le panneau la montre dans sa
prévisualisation **sans une ligne de front**. Côté aller la source est l'index zip du wheel (lire ne pose rien : la
prévisualisation reste strictement idempotente) ; côté retour, le venv cible, déjà sur le disque. Une dégradation
annoncée n'est pas un cap silencieux — une dégradation tue en serait un.

## apply() — le geste complet, trois issues toutes explicites
`src/forgemaster/apply_update.py:506` · appelé par `main` · retourne `(rc, verdict, détails)`
Sept étapes : venv neuf → sonde en isolation → **arrêt + instantané à froid** → bascule du lien + redémarrage → **repose
des cartes de l'édition** → vérification en vivant → retour arrière automatique si elle échoue. La repose est **avant**
le verdict, et c'est contraint : le juge de rendu jugerait sinon une instance mi-montée. Trois issues : **posée** (rc 0),
**refusée avant bascule** (rc 1, rien n'a bougé), **revenue en arrière** (rc 1, l'instance est telle qu'avant). Le retour
automatique rebascule le lien, restaure, **puis repose les cartes de l'ancien venv** — sans ça il fabriquerait lui-même
l'état inédit qu'il existe pour interdire ; si la restauration échoue, il **re-bascule EN AVANT** (rc 2) — le binaire neuf
sait lire ces données-là, et un binaire ancien sur des données neuves est le seul état interdit (pas de repose sur cette
branche : le lien est reparti sur le bleu, dont les cartes ont déjà été posées).

## rollback() — le symétrique de l'aller, pas un second mécanisme
`src/forgemaster/apply_update.py:596` · appelé par `main` · retourne `(rc, verdict, détails)`
Six étapes, chacune réutilisant une fonction que `apply` exerce déjà. **L'ordre est contraint et non négociable : le
...
`src/forgemaster/apply_update.py:722` · appelé par `rollback` (étape 1/6) · lève `UpdateFailed`
...
postérieurs. L'étape [3/6] **re-vérifie** ensuite l'existence de la cible, la prise ayant eu lieu entre-temps.

## _restore() — rendre `False` plutôt qu'avaler l'échec
`src/forgemaster/apply_update.py:780` · appelé par `apply` et `rollback` · retourne un booléen
...
appelant ne peut pas ignorer. Le drapeau `FORCE_FLAG` (`apply_update.py:72`) n'est passé que si `_supports_flag`
...
`src/forgemaster/apply_update.py:819` · `src/forgemaster/apply_update.py:841` · appelés par `apply`, `rollback`, `probe_isolated`
...
silence qui remonterait à l'utilisateur comme diagnostic. `_unservable_detail` (`apply_update.py:872`) exige la forme
...
`src/forgemaster/apply_update.py:64` · lu par `_purge_venvs` et par `snapshot.KEEP`
...
`src/forgemaster/apply_update.py:913` · appelé par `apply` (seulement après un vivant vert) · aucun retour
...
La lecture se fait en quatre temps, et l'**ordre** est le contrat : `parse_envelope` (`:195`) lit l'enveloppe et
**décode** le payload sans le parser → `verify_envelope` (`:232`) exige que le `key_id` soit dans le jeu embarqué
**et** que la signature vérifie sous **cette** clé (jamais « essaie toutes les clés ») → `parse_announce` (`:269`)
n'est appelée qu'après. C'est *parse-after-verify* : parser d'abord ferait du parseur la première surface d'attaque.
`trust_root` (`:148`) **re-dérive** le `key_id` de chaque clé et refuse un fichier qui ment — un identifiant dérivé
qu'on ne recalcule pas n'est qu'un nom.

`refresh` (`:396`) orchestre et **ne lève jamais** : un canal muet ne fait pas tomber un daemon. Il **court-circuite
...
`run_channel_poll` (`:455`) est l'idiome sleep-loop de `run_reaper`, avec **une** différence structurelle : le tirage
...
vient de redémarrer est le moment où quelqu'un veut savoir. `cli_check` (`:618`) est le même tirage à la demande, pour
