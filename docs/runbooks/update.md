...
`src/forgemaster/update.py:272` · appelé par `preflight_rollback` · retourne `(venv cible, motif de refus)`
...
`src/forgemaster/update.py:299` · appelé par `_cible_utilisable` · retourne le venv ou `None`
...
`src/forgemaster/update.py:320` · appelé par `_preflight_service` · retourne `(binaire, host, port)` · lève `UpdateRefused`
...
`src/forgemaster/update.py:341` · `src/forgemaster/update.py:380` · appelés par `cli_dispatch` · seul contenu de `--dry-run`
...
`src/forgemaster/update.py:399` · appelé par `launch` et par `routes/update._lance` · rend `{run, unit, ok, detail}`
...
`src/forgemaster/update.py:478` · appelé par `cli_dispatch` · retourne le rc de `follow` (ou 0 si `--detach`)
...
`src/forgemaster/update.py:855` · appelé par `launch` · rend le préfixe `systemd-run` de la commande
...
`src/forgemaster/update.py:555` · appelés par `routes/update` · lecture pure, aucun effet
...
`src/forgemaster/update.py:667` · appelés par `routes/update` et `cli_dispatch` · écriture bornée, purge à l'écriture
...
`forgemaster update wheels` (`_cli_wheels`, `src/forgemaster/update.py:963`) — parce qu'une politique de rétention qui
...
`src/forgemaster/update.py:504` · appelé par `launch` · retourne le rc lu dans `result.json`
...
`src/forgemaster/update.py:890` · appelés par `cli_dispatch` et par `routes/update._plan` · retournent les verdicts, ou `[]`
...
`src/forgemaster/update.py:933` · appelé par `cli._h_update` (routé par `_HANDLERS`) · retourne le code de sortie
