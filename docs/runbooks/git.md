...
`src/cockpit/git/internal.py:217` · implémente GitBackend · instancié par l'orchestration (gate/merge, dispatch)
...
`src/cockpit/git/internal.py:64` · parser **PUR** · appelé par `InternalGit.status()` (`status --porcelain=v2 -b`)
...
`src/cockpit/git/internal.py:109` · parser **PUR** · appelé par `InternalGit.log()` (`log --oneline`)
...
`src/cockpit/git/internal.py:120` · classifieur **PUR** · consommé par l'orchestration de push / `clone_sot`
...
`src/cockpit/git/internal.py:137` · prédicat **PUR** · `PROTECTED_BRANCHES = ("main", "dev")` (l. 31)
...
`src/cockpit/git/internal.py:155` · rollup **PUR** · au-dessus de `_branch_sync_state()` (l. 142) · consommé par `remote_divergence`
...
`src/cockpit/git/internal.py:173` · pur (compose un env, **ne mute pas `os.environ`**) · appelé par `merge_writeback`, `commit_worktree`, `_seed_base`
...
`src/cockpit/git/internal.py:186` · pur (**ne mute ni `base` ni `os.environ`**) · composé au-dessus de `writeback_env` par `merge_writeback`/`_authed_env`
...
`src/cockpit/git/internal.py:203` · contextmanager · `flock LOCK_EX` sur `<sot>/cockpit-worktree.lock` (`_LOCK_NAME`, l. 32) · enveloppe `add_worktree`/`remove_worktree`
