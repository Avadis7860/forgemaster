...
`src/forgemaster/git/internal.py:130` · parser **PUR** · appelé par `InternalGit.log()` (`log --oneline`)
...
`src/forgemaster/git/internal.py:141` · classifieur **PUR** · consommé par l'orchestration de push / `clone_sot`
...
`src/forgemaster/git/internal.py:158` · prédicat **PUR** · `PROTECTED_BRANCHES = ("main", "dev")` (l. 31)
...
`src/forgemaster/git/internal.py:176` · rollup **PUR** · au-dessus de `_branch_sync_state()` (l. 142) · consommé par `remote_divergence`
...
`src/forgemaster/git/internal.py:194` · pur (compose un env, **ne mute pas `os.environ`**) · appelé par `merge_writeback`, `commit_worktree`, `_seed_base`
...
`src/forgemaster/git/internal.py:207` · pur (**ne mute ni `base` ni `os.environ`**) · composé au-dessus de `writeback_env` par `merge_writeback`/`_authed_env`
...
`src/forgemaster/git/internal.py:225` · contextmanager · `flock LOCK_EX` sur `<sot>/forgemaster-worktree.lock` (`_LOCK_NAME`, l. 32) · enveloppe `add_worktree`/`remove_worktree`
...
`src/forgemaster/git/identity.py:33` · **PUR** · placé sous `git/` (neutre, aucun cycle) · deux consommateurs : commit worker (`dispatch`, rôle `worker`), writeback merge (`gate/merge`, rôle `writeback`)
...
`src/forgemaster/git/github.py:13` · scaffoldé, **non implémenté** · signatures figées par `GitBackend`
...
- `BlobTooLargeError` (`internal.py`) : refus explicite de lire un blob au-delà du plafond — un fichier énorme se dit, il ne se tronque pas en silence.
