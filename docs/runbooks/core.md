...
`forgemaster.core` regroupe les briques PURES et sans état sur lesquelles s'appuient les couches hautes (dispatch, gate, git, projects). Invariant clé : **zéro shell implicite, zéro chemin d'hôte codé en dur, zéro littéral d'id** — tout ce qui touche l'I/O locale, le système de fichiers ou l'identité passe par ici pour rester injectable en test et borné en prod. Le seam de transport (`run`) remplace le `ssh dev@<ip>` du legacy par une exécution locale `subprocess`.

## run() — exécution locale bornée, remplace le `ssh dev@ip` legacy
`src/forgemaster/core/run.py:52` · appelé par dispatch/reviewer, gate/toolchain, gate/verify, webbuild, mcp/client, daemon/app
...
`src/forgemaster/core/run.py:98` · appelé par dispatch/worker
Comme `run` mais argv en LISTE uniquement (pas de `shell`), et écrit le stdout **ligne par ligne** dans `out_path` (open+write+flush par ligne) au lieu de ne le capturer qu'à la fin → rend le transcript d'un worker `claude -p --output-format stream-json` suivable en direct (le pont `dispatch/stream` tail ce fichier). Le stdout complet reste accumulé et rendu dans le `RunResult` final. Invariants : `timeout` honoré **même sans aucune sortie** (thread lecteur + `proc.wait(timeout)`, kill → `RunTimeout`) ; stdout ET stderr drainés en threads séparés → pas de deadlock de pipe.
Trois seams, tous là pour rendre un worker **tuable** et **interruptible** — c'est ce qui remplace le `pgrep`/`kill -0` fragile par un handle explicite : `new_session=True` place le process dans sa **propre session** (`setsid`, donc leader de groupe) et un abort peut le tuer **en groupe** (`os.killpg`) sans toucher l'orchestrateur ; `on_spawn(pid)` est appelé juste après le spawn (le dispatch y persiste le pid en DB → l'abort cross-process le retrouve, et `reconcile` sait qui respire encore) ; `on_line(line) -> raison|None` est un **prédicat d'abort injecté** appelé sur chaque ligne de stdout — la première raison truthy tue le process et fait lever `RunSpin`. Le cœur reste générique : il ne sait pas ce qu'est un « spin », la sémantique de non-progression appartient à l'appelant (`dispatch.make_spin_detector`) ; une exception du prédicat n'interrompt jamais le pompage. Sortie : `RunResult`.

## RunResult / RunError / RunTimeout — le résultat structuré et ses levées
`src/forgemaster/core/run.py:39` (RunResult), `:20` (RunError), `:28` (RunTimeout)
`RunResult` (dataclass frozen) porte `argv`/`returncode`/`stdout`/`stderr` + propriété `ok` (rc==0). `RunError(RuntimeError)` : levée par `run(check=True)` sur rc≠0, encapsule le `RunResult` (attribut `.result`) et formate un message `rc=… pour <argv>: <stderr tronqué 200c>`. `RunTimeout(RuntimeError)` : levée par les deux fonctions quand le `timeout` est dépassé (process tué dans `run_streaming`). `RunSpin` : levée par `run_streaming` seul, quand le prédicat `on_line` a rendu une raison — un worker qui tourne sans progresser est **tué et nommé**, il n'attend pas son timeout. À distinguer d'un `RunTimeout` : le premier dit « il n'avance plus », le second « il n'a pas fini à temps ».

## safe_path() — bornage anti-traversal d'un chemin sous une racine
`src/forgemaster/core/fs.py:16` · appelé par terminal/pty
...
`src/forgemaster/core/fs.py:28` (iter), `:36` (read), `:44` (write)
...
`src/forgemaster/core/ids.py:27` · appelé par projects/registry, dispatch/jobs, dispatch/reviewer, dispatch/ports, projects/deployments, roadmap/model
...
`src/forgemaster/core/ids.py:15` (is_slug), `:20` (ensure_slug) · appelé par projects/registry, roadmap/model
