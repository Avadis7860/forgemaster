...
`src/cockpit/dispatch/worker.py:51` · appelé par `dispatch_next()` et `dispatch_reviewer()`
...
`src/cockpit/dispatch/worker.py:114` · appelé par `dispatch_next()` et via `worker.parse_headless_result` dans `dispatch_reviewer()`
...
`src/cockpit/dispatch/worker.py:169` · appelé par `cli_dispatch` (route `cockpit dispatch <feature>`) et l'orchestrateur
...
`src/cockpit/dispatch/worker.py:141` · appelé par `dispatch_next()` dans la branche run-réussi uniquement
...
`src/cockpit/dispatch/reviewer.py:49` · appelé par `dispatch_reviewer()`
...
`src/cockpit/dispatch/reviewer.py:101` · appelé par `dispatch_reviewer()` sur `parsed["result"]`
...
`src/cockpit/dispatch/reviewer.py:127` · appelé par `dispatch_reviewer()` avant tout spawn
...
`src/cockpit/dispatch/reviewer.py:140` · appelé par `cli_dispatch` (route `cockpit gate review-dispatch <feature>`), auto post-travail
