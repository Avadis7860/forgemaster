...
2. **Les 4 étages couvrent le critère binaire.** (1) un projet se déploie sur `main` **ET** `dev` → 2
   ...
   même temps → **4 compose-projects** (`forgemaster-<slug>-<branch>`), **4 ports disjoints**, aucun conflit, les 4
...
4. **Feature-verified ancré au SHA de HEAD (forgemaster).** Le runner Playwright **goto-only** (`render_check.js`)
...
5. **Rejouable, teardown garanti.** `FORGEMASTER_HOME` jetable ; en `finally`, `down` des déploiements + arrêt du
...
## Prouvé live (podman-rootless, 2026-07-13, forgemaster `19f0be2`)
