...
Le cockpit **réimplémente** l'orchestrateur legacy (`services/aggregator/`), il ne le copie pas. La
...
| # | Dette du legacy | Origine | Refactor décidé (cockpit) | Emplacement cible |
|---|---|---|---|---|
...
| **4** | **Chemins d'hôte en dur** : `/home/dev` (`DEV_HOME`), `SSH_KEY_PATH`, `SSH_KNOWN_HOSTS`. | `terminal.py`, `server.py` | Racines par **config** (`COCKPIT_HOME`/`COCKPIT_PROJECTS_ROOT`) ; `safe_path(root=…)` paramétré. | `config.py`, `core/fs.py` |
...
| **12** | **Lock in-process** : ne couvre pas des relais/process concurrents (2 features simultanées). | dispatch legacy | **`flock` sur un fichier du `.git` PARTAGÉ** (`.git/cockpit-worktree.lock`, fd dédié). | `dispatch/worktree.py` |
