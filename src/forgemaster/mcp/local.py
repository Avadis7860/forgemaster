"""mcp.local — l'instance forgemaster-catalogs que CE forgemaster fait tourner (topologie co-installée).
...
seule la seconde existait (`FORGEMASTER_MCP_ENDPOINT` vers un serveur d'ailleurs).

Calqué sur `forgemaster.tools` (l'outillage hôte-niveau), volontairement et jusque dans le détail :
...
cap silencieux). Le forgemaster ne clone aucun corpus, jamais : le nôtre ne se distribue pas, et mettre le
...
serveur est-il installé sous `$FORGEMASTER_HOME/mcp/venv` ? l'endpoint consommé est-il en loopback ? — et leur
...
# le défaut du serveur lui-même (forgemaster, lui, sert sur 8700)
DEFAULT_PORT = 8080
...
# propre forgemaster, et le symptôme (401) ne nommerait pas la cause.
...
def mcp_root(settings: Settings) -> Path:
    """Racine de l'instance MCP co-installée : `$FORGEMASTER_HOME/mcp/`."""
    return settings.home / "mcp"


def mcp_venv(settings: Settings) -> Path:
    """Venv Python DÉDIÉ du serveur — ni celui du forgemaster, ni celui des outils. Trois venvs parce que
...
def endpoint_url(port: int = DEFAULT_PORT, *, host: str = LOOPBACK_HOST) -> str:
    """L'URL que le forgemaster consommera. Le chemin `/mcp` est le contrat du serveur (c'est lui qui liste
    les
...
def render_env(*, port: int, data_root: Path, secret: str) -> str:
    ...
    return (
        f"# {SERVER_UNIT}.env — posé par `forgemaster mcp install`. Porte le secret HS256 : chmod 600.\n"
        "# Ne pas committer. Régénéré à chaque `forgemaster mcp install`, jamais édité à la main.\n"
...
def render_unit(settings: Settings, *, data_root: Path, scope: str = "user") -> str:
    ...
    return (
        "[Unit]\n"
        f"Description={SERVER_UNIT} — corpus MCP typé, co-installé avec ce forgemaster (loopback + JWT)\n"
        ...
        if not endpoint:
            return {"topology": "none", "sha": None, "endpoint": None,
                    "reason": "aucun endpoint MCP configuré — instance sans corpus à interroger "
                              "(`forgemaster mcp install` pour co-installer, `forgemaster mcp wire` pour un "
                              "distant)"}
...
def install(settings: Settings, *, data_root: str | Path, port: int = DEFAULT_PORT,
    ...
    """Co-installe le serveur MCP sur cet hôte (IDEMPOTENT, FAIL-LOUD) et câble le forgemaster dessus.
    ...
    **réutilise** — le régénérer invaliderait les jetons du serveur qui tourne, à chaque ré-exécution d'une
    ...
    if not root.is_dir():
        raise McpInstallError(
            ...
            "Le co-install pose un LECTEUR de corpus ; le corpus est ta donnée, le forgemaster n'en clone "
    ...
    if token_ref:                                       # dépôt encore privé (P6.4 le publie) → auth git
        from forgemaster.git.internal import credential_env
        from forgemaster.secrets import cred_resolver
    ...
    try:
        ...
        raise McpInstallError(f"câblage du forgemaster impossible après l'install — {exc}") from exc
...
def cli_install(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster mcp install` : co-installe le serveur, puis IMPRIME le geste systemctl restant (on
