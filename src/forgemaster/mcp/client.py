"""mcp.client — client MCP **runtime** du forgemaster : parle au serveur `forgemaster-catalogs` **depuis le
...
en direct. Import `fastmcp` **paresseux** (le socle forgemaster ne le tire pas au chargement).
...
from forgemaster.provision.mcp import (
...
from forgemaster.secrets.jwt import mint_hs256
...
# sub du JWT pour la résolution blueprint (serveur valide aud/iss/signature)
_SUBJECT = "forgemaster:board"
_SUBJECT_CAPITAL = "forgemaster:capital"  # sub du JWT pour le parcours du capital-token (surface Landing)
...
def blueprint_resolver(settings: Settings, *, secret_ref: str | None = None,
    ...
    **seams** (défauts : env `FORGEMASTER_MCP_JWT_SECRET_REF`, `current_endpoint()`, `cred_resolver`, la
...
def capital_browser(settings: Settings, *, secret_ref: str | None = None,
    ...
    Landing. Mêmes **seams** que `blueprint_resolver` (défauts : env `FORGEMASTER_MCP_JWT_SECRET_REF`,
