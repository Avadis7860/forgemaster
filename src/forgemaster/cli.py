"""cli — la porte unifiée `forgemaster`. Le câblage argparse est **complet et figé dès la structure** (les
...
def build_parser() -> argparse.ArgumentParser:
    ...
    from forgemaster.provision import list_valid_types
    parser = argparse.ArgumentParser(prog="forgemaster", description="Forge/orchestrateur local de projets.")
    parser.add_argument("--version", action="version", version=f"forgemaster {__version__}")

    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--home", help="racine d'état du forgemaster (défaut: $FORGEMASTER_HOME ou "
    "~/.forgemaster)")
    ...
    p_tools = sub.add_parser("toolchain", parents=[common],
    ...
    pi = sub.add_parser("install-service", parents=[common],
                        help="installer une unité systemd pour `forgemaster serve` (self-hosted)")
    ...
    pmw = p_mcp_sub.add_parser("wire", parents=[common],
                               help="poser la ref du secret + l'endpoint MCP dans forgemaster.env")
    ...
    pmi.add_argument("--data-root", required=True,
                     help="racine du corpus à servir (TA donnée — le forgemaster n'en clone aucune)")
...
def _autoload_env(args: argparse.Namespace) -> None:
    """Charge `$FORGEMASTER_HOME/forgemaster.env` dans `os.environ` AVANT de résoudre les Settings — parité
    CLI ↔
    service (le service lit l'`EnvironmentFile`, la CLI doit voir la même config, dont le câblage MCP). Sinon
    un `forgemaster dispatch` en shell perdait le MCP en silence. Home résolu comme `_settings` (flag > env >
    défaut) ; le fichier ne surcharge jamais une clé déjà dans l'environnement réel."""
    import os
    from pathlib import Path

    from forgemaster.config import DEFAULT_HOME, ENV_HOME
    from forgemaster.service import load_env_file
    home = getattr(args, "home", None) or os.environ.get(ENV_HOME) or DEFAULT_HOME
    load_env_file(Path(os.path.expanduser(str(home))) / "forgemaster.env")
...
def _h_refix(settings: Settings, args: argparse.Namespace) -> int:
    from forgemaster.dispatch import refix
...
def _h_scaffold(settings: Settings, args: argparse.Namespace) -> int:
    from forgemaster.provision import reseed
...
def _h_inspire(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster inspire <projet> <template>` : applique un template UI de référence servi comme
    cible
    ...
    from forgemaster.daemon.app import web_dist_dir
    from forgemaster.db import store
    from forgemaster.design.apply import apply_template
    ...
    print(f"template « {report['template']} » appliqué à {report['project']} → feature {report['feature']} "
          ...
          f"Dispatch la customisation : `forgemaster dispatch {report['project']}/{report['feature']}`.")
    return 0


def _h_upload(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster upload <projet> <chemin> [--dest <slug>] [--feature <f>]` : dépose le fichier local
    dans
    ...
    from forgemaster.content.ingest import ingest_upload
    from forgemaster.db import store
    src = Path(args.path)
    try:
        data = src.read_bytes()
...
def _h_gate(settings: Settings, args: argparse.Namespace) -> int:
    from forgemaster.dispatch import reviewer
    from forgemaster.dispatch import woaw as woaw_dispatch
    from forgemaster.gate import review, toolchain, verify
...
def _h_serve(settings: Settings, args: argparse.Namespace) -> int:
    from forgemaster.daemon import app
...
def _h_setup(settings: Settings, args: argparse.Namespace) -> int:
    ...
    if web is None:                                      # install wheel pure : l'UI est déjà empaquetée
        print("UI déjà empaquetée (install wheel) — rien à builder. Lance `forgemaster serve`.")
        return 0
    try:
...
def _h_install_service(settings: Settings, args: argparse.Namespace) -> int:
    ...
    if link.is_symlink():
        print(f"lien stable         → {link} → {link.resolve()} "
              f"(l'unité le lance ; c'est lui que `forgemaster update apply` bascule)")
    else:
        print(f"lien stable         → non posé ({link}) : ce forgemaster ne tourne pas dans un venv qui "
              f"porte la commande `forgemaster`. `forgemaster update apply` refusera — c'est voulu.")
