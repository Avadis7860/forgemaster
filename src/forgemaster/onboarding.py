...
RIEN par lui-même : il compose le **store de secrets actif** (`forgemaster.secrets`) et le **registre de
...
from forgemaster import __version__, auth, build_provenance
from forgemaster.config import Settings
from forgemaster.db import store as db_store
from forgemaster.projects import registry
from forgemaster.provision import mcp
from forgemaster.secrets import SecretNotFound, SecretStore, SecretUnsupported, build_store
...
def status(conn: sqlite3.Connection, secret_store: SecretStore,
    ...
    - `build` : provenance + fraîcheur du wheel installé (`{version, sha, committed_at, comparable, stale,
      behind_by, missing_types}`, cf. `build_provenance`). Signal honnête anti cap-silencieux : un
      forgemaster en retard sur son SoT local se DÉCLARE, jamais faux-vert. `build_state` injectable
      (tests) ; défaut = détection live via `settings` (miroir absent ⇒ `comparable=False`) ; sans
      `settings`, tampon seul."""
...
def wire_mcp(settings: Settings, *, secret: str | None = None, ref: str | None = None,
    ...
    référence opaque du secret HMAC + l'endpoint dans `forgemaster.env` ET dans l'`os.environ` du daemon
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster onboard <action>` : `status` (défaut) affiche ce qui manque ; `link` lie un token
        ...
        if action == "status":
            ...
            if st["first_run"]:
                print("  ▸ instance neuve : aucun projet encore. Crée ton 1er projet "
                      "(`forgemaster project create <slug>` ou le wizard web `/setup`).")
            ...
            if mcp_st.get("wired"):
                ...
                cible = mcp_st.get("endpoint") or (
                    "AUCUN ENDPOINT — `forgemaster mcp wire --endpoint <url>`")
                print(f"  ✅ corpus capital (MCP) — câblé ({cible})")
            else:
                print("  ℹ️ corpus capital (MCP) — à connecter (wizard /setup ou `forgemaster mcp wire`) ; "
