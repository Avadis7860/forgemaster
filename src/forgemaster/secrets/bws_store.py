"""secrets.bws_store — `BwsStore` : adaptateur Bitwarden Secrets Manager (store OPTIONNEL,
`forgemaster[bws]`).
...
class BwsStore:
    ...
    def _default_client_factory(access_token: str, api_url: str, identity_url: str):  # type: ignore[no-untyped-def]
        try:
            ...
            raise SecretStoreError(
                "SDK Bitwarden absent — installe l'extra `forgemaster[bws]` pour activer le store BWS."
            ) from e
