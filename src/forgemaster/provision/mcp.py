...
(`MCP_JWT_SECRET`) est résolu par le coffre du forgemaster à l'usage ; absent/illisible → **no-op honnête**
(le
...
from forgemaster.secrets import SecretNotFound, SecretUnsupported, build_store, cred_resolver
from forgemaster.secrets.jwt import b64url_decode, mint_hs256
from forgemaster.service import set_env_keys

ENV_MCP_ENDPOINT = "FORGEMASTER_MCP_ENDPOINT"


def current_endpoint() -> str | None:
    """L'endpoint MCP EFFECTIF, résolu à l'APPEL (jamais gelé) : `os.environ['FORGEMASTER_MCP_ENDPOINT']` —
    que
    ...
    **`None` = aucun endpoint configuré, et c'est un état NORMAL** — une install sans corpus privé n'a pas
    ...
    que nous ne l'exécutait. Un forgemaster n'a **pas** d'instance MCP par défaut ; l'absence se dit (`None`),
...
# Référence (dans le coffre du forgemaster) du secret HMAC partagé qui signe les JWT du MCP. Absent → pas de
# MCP.
ENV_MCP_JWT_SECRET_REF = "FORGEMASTER_MCP_JWT_SECRET_REF"
...
def render_mcp_config(token: str, *, endpoint: str | None = None) -> dict:
    ...
    if not ep:
        raise ValueError(
            ...
            "câble une instance (`forgemaster mcp wire --endpoint <url>`) ou passe `endpoint=`.")
    return {
        "mcpServers": {
            MCP_SERVER_LABEL: {
                "type": "http",
...
def inject_mcp_config(worktree: Path, settings: Settings, *, slug: str,
    ...
    `''` si absent/illisible). Mint un JWT scopé `sub=forgemaster:<slug>` (aud/iss du contrat serveur).
    **No-op
    ...
    if len(secret) < 32:                                   # secret absent / illisible / mal configuré
        return None
    token = mint_hs256(f"forgemaster:{slug}", secret, audience=MCP_AUDIENCE, issuer=MCP_ISSUER,
...
def check_lifecycle(settings: Settings, *, now: int, window_s: float = _RUN_WINDOW_S,
    ...
    """Diagnostic **déterministe** (zéro réseau) du cycle de vie du token MCP, pour `forgemaster doctor`.
    Retour :
    `{configured, healthy, reason, exp, stale}`.

    - **non câblé** (pas de `FORGEMASTER_MCP_JWT_SECRET_REF`) → `configured=False, healthy=True` (install
    public
    ...
    if not current_endpoint():
        return {"configured": True, "healthy": False, "exp": None, "stale": [],
                "reason": f"aucun endpoint MCP ({ENV_MCP_ENDPOINT}) — ref de secret posée mais pas de "
                          "cible : le dispatch n'injectera aucun `.mcp.json` (`forgemaster mcp wire "
                          "--endpoint`)"}
    resolve = resolver or cred_resolver(settings)
    secret = resolve(ref)
    if len(secret) < 32:
    ...
    probe = mint_hs256("forgemaster:doctor", secret, audience=MCP_AUDIENCE, issuer=MCP_ISSUER,
                       ttl_seconds=_TTL_SECONDS)
    stale: list[dict] = []
...
def wire(settings: Settings, *, secret_ref: str | None = None, secret: str | None = None,
    ...
    écrit une **référence opaque** (JAMAIS le secret en clair) + l'endpoint dans `forgemaster.env`, de sorte
    que
    le prochain dispatch injecte un `.mcp.json` valide (`inject_mcp_config`). Retourne la ref posée. Cœur
    partagé par la CLI (`forgemaster mcp wire`) et la route onboarding (wizard).
    ...
    **Exactement une** voie parmi : `secret` (valeur brute POSSÉDÉE — le wizard POST le secret → `store.put`
    ...
    if not ep:
        raise MCPWireError(
            ...
            "un forgemaster n'a pas d'instance forgemaster-catalogs par défaut.")
    ...
    set_env_keys(settings.home / "forgemaster.env", {ENV_MCP_JWT_SECRET_REF: ref, ENV_MCP_ENDPOINT: ep})
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster mcp wire` : câble le MCP de corpus (délègue à `wire()`). Deux voies exclusives
    ...
    print(f"✅ MCP câblé → {endpoint}  (ref opaque posée dans {settings.home / 'forgemaster.env'}).")
    print("   redémarre le service pour recharger l'EnvironmentFile : "
          "`systemctl restart forgemaster` (ou `--user`).")
    return 0
