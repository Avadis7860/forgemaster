...
À la **première session** d'un browser-game neuf, lancé par `forgemaster interview <projet>`. Le projet est semé
...
2. La roadmap porte les **features d'amorçage jouable** — typiquement scaffold serveur (backend), schémas
   ...
   `acceptance`), et `forgemaster roadmap check <projet>` est **vert**.
...
```bash
forgemaster roadmap add-feature <projet> scaffold-serveur --facet backend --title "Scaffold serveur"
forgemaster roadmap add-feature <projet> schemas-partages  --facet backend --title "Schémas partagés"
forgemaster roadmap add-feature <projet> jalon-jouable     --facet frontend --title "Premier jalon jouable"
forgemaster task add <projet>/<feature> <task> --acceptance "Critère binaire, testé : …"
forgemaster roadmap check <projet>          # VERT (0 issue)
```
...
en `done` — ne marque pas les tasks toi-même. `forgemaster run <projet>` draine ensuite les features jouables.
