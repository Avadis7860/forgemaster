...
3. **Contrat de run** — nginx écoute `0.0.0.0:8000` ; `compose.yaml` publie `${FORGEMASTER_PORT:?}:8000` (échec
   bruyant si le port n'est pas injecté) ; aucun `volumes:`/`networks:` ; aucun nom de projet en dur.
4. **Secrets** — jamais dans l'image ni le compose ; passent par le mécanisme du forgemaster.
