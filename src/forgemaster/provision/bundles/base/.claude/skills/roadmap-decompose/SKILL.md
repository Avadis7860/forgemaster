...
features) **ou** s'automatise (`forgemaster run` draine et parallélise) — aux mêmes artefacts.
...
2. **Découpe en features par facette ET par indépendance.** Une frontière de feature tombe là où (a) la
   ...
   **indépendantes prêtes** tournent **en parallèle** (`forgemaster run` ⇒ N workers) — les rendre indépendantes
...
## En pratique (forgemaster ou manuel)

```bash
# Avec le forgemaster (in-repo : features + tasks dans la roadmap du projet)
forgemaster roadmap add-feature <projet> <feature> --facet backend --depends-on <feat-prereq>
...
# jamais en éditant forgemaster.db à la main : les verbes valident (refus dangling/cycle/self) et écrivent atomiquement.
forgemaster roadmap set-deps <projet> <feature> --depends-on <feat-prereq...>   # REMPLACE les deps inter-feature
forgemaster task set-deps <projet>/<feature> <task> --depends-on <t-prereq...>  # REMPLACE les deps intra-task
forgemaster roadmap show <projet>          # relire le DAG
forgemaster roadmap check <projet>         # prouver : DoD/DAG/facettes OK (0 issue = drainable)
forgemaster run <projet> --max-parallel 2  # drainer + paralléliser les features indépendantes prêtes
```

Sans forgemaster, les mêmes artefacts se tiennent à la main (roadmap in-repo + boucle `work-loop` feature par
...
- **Édition du DAG en raw-SQL** — corriger une dépendance en ouvrant `forgemaster.db` (`sqlite3`/`UPDATE`) contourne
...
`work-loop` (manuel) ou `forgemaster run` (automatisé).
