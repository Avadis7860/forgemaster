# CLAUDE.md — projet auto-travaillable (semé par le forgemaster)
...
> pas ici. Ce projet a été créé par le forgemaster avec un toolkit qui le rend **travaillable seul** (un clone
> suffit) ; le forgemaster ne fait qu'**automatiser** la même boucle, aux **mêmes invariants**.
...
- **Anti-patterns** (jamais) :
  - inventer une signature d'API « de mémoire » avant un import non trivial → **si un MCP de corpus est câblé**
    (`forgemaster mcp wire`), interroge le silo de la lib (`query(type=tech, scope=<silo>)`) ; sinon **lis** la
