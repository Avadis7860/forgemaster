...
def resolve_facet(root: Path, feature_facet: str | None) -> str:
    ...
    `.forgemaster/bundle.toml` de la worktree, sinon `doc`. Fail-soft (manifeste absent/illisible →
    fallback)."""
    if feature_facet:
        return feature_facet
    manifest = Path(root) / ".forgemaster" / "bundle.toml"
    if manifest.is_file():
        try:
...
def resolve_facet_model(root: Path, facet: str) -> str | None:
    ...
    `[bundle.facet_models]` du `.forgemaster/bundle.toml` de la worktree (`facet → alias/id`), symétrique de
