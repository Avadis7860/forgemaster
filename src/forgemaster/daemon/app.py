...
def web_dist_dir() -> Path:
    """Emplacement du build SPA, dans l'ordre : override `FORGEMASTER_WEB_DIST` → dist **empaquetée** dans le
    paquet (`forgemaster/_web_dist`, livrée par le wheel — turnkey, aucun Node requis) → layout **source**
    (`<repo>/web/dist`, dev/éditable, buildé par `forgemaster setup`). Retourne le 1er chemin dont
    `index.html`
    existe ; à défaut, le chemin source (référence citée par le placeholder fail-loud de `_mount_spa`)."""
    override = os.environ.get("FORGEMASTER_WEB_DIST")
    if override:
        return Path(override)
    here = Path(__file__).resolve()
    packaged = here.parents[1] / "_web_dist"             # forgemaster/_web_dist (livré par le wheel)
...
def build_app(settings: Settings) -> FastAPI:
    ...
    from forgemaster.daemon.routes import (
        alerts,
        bootstrap,
        bundles,
        capital,
    ...
    async def _lifespan(_app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        # Canal de MAJ : au démarrage PUIS à intervalle (un daemon cockpit vit des semaines — un contrôle au
        # boot seul ne notifierait jamais rien). Même forme que le reaper, et pour la même raison : une tâche
    ...
    app = FastAPI(title="forgemaster", version=__version__, lifespan=_lifespan)
    ...
    def get_ws_token() -> dict:
        ...
        peut donc pas forger `forgemaster.token.<v>`. Le token seul ne suffit pas non plus (barrière
        Origin)."""
    ...
    from forgemaster.content.upload import UploadTooLarge, UploadTypeRejected
...
_MISSING_UI_HTML = (
    "<!doctype html><meta charset=utf-8><title>forgemaster — UI non buildée</title>"
    ...
    "<h1>forgemaster — l'UI n'est pas encore buildée</h1>"
    "<p>Le daemon tourne (l'API répond sur <code>/api</code> et <code>/health</code>), mais l'interface web "
    "n'a pas été construite.</p>"
    "<p><b>Depuis les sources :</b> lance <code>forgemaster setup</code> puis recharge cette page.</p>"
...
def _mount_missing_ui_placeholder(app: FastAPI, dist: Path) -> None:
    ...
    logging.getLogger("forgemaster").warning(
        "UI non buildée (dist absente : %s) — API-only. Build : `forgemaster setup` (from-clone) "
