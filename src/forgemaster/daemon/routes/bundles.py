...
plus ni moins → introspection honnête « qu'embarque le forgemaster » avant distribution. Goto-safe : le runner
...
from forgemaster.provision import list_valid_types, load_bundle
...
def _classify(path: str) -> str:
    ...
    if path.startswith(".claude/facets/"):
        return GROUP_METHOD
    if path.startswith(".forgemaster/") or path in _DEPLOY_FILES:
        return GROUP_DEPLOY
