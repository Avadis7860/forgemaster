...
- `/ws/interview/{project}` → **interview de socle** (`bash -lc 'exec forgemaster interview <project>'`, clé
  `interview:<project>`). Le process du PTY **EST** l'interview (plus de commande tapée dans un shell partagé
  gaté sur `fresh` — l'ancien modèle cassait dès que le shell du projet persistait). `forgemaster interview`
