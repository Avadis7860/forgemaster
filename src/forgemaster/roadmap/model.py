...
`.forgemaster/roadmap.yaml` (schéma figé — cf. docs/schema-contract.md § roadmap.yaml).
...
from forgemaster.provision import BundleError, read_bundle_manifest
...
def set_feature_deps(conn: sqlite3.Connection, *, ref: str, depends_on: list[str]) -> dict:
    ...
    from forgemaster.roadmap import resolver  # import paresseux : resolver importe déjà model (cycle)
...
def add_task(conn: sqlite3.Connection, *, feature_ref: str, slug: str, title: str | None = None,
    ...
    est routée vers un terminal interactif (`forgemaster interview`) au lieu d'un worker `claude -p`
    headless."""
...
def set_task_deps(conn: sqlite3.Connection, *, feature_ref: str, slug: str, depends_on: list[str]) -> dict:
    ...
    from forgemaster.roadmap import resolver  # import paresseux (cf. set_feature_deps)
...
def to_yaml(project_slug: str, features: list[dict]) -> str:
    """Sérialise une roadmap (features + leurs tasks) au contrat `.forgemaster/roadmap.yaml`. PUR."""
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster roadmap <action>` (add-feature|set-deps|show)."""
