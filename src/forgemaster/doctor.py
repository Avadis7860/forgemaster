...
en shell : `forgemaster doctor; echo $?` (rc 0 = tout présent, 1 = au moins un manquant).
...
from forgemaster.provision import _BUNDLES_DIR
from forgemaster.provision import mcp as mcp_provision
from forgemaster.tools import HOST_TOOLS, missing_bins, required_bins, tools_bin, tools_env
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster doctor` : imprime le rapport de présence par type/facette et retourne 0 si tous les
    ...
    if all_missing:
        print(f"🔴 outillage INCOMPLET — pose les manquants (`forgemaster toolchain install`) : "
...
def _report_runtime(settings: Settings) -> bool:
    ...
    `podman-compose` standalone a pour moteur `podman`) — est requis pour `forgemaster deploy` (P2). Absent →
    🔴
    **bloquant** (le deploy crasherait) ; présent → ✅."""
    from forgemaster.runtime.backend import compose_engine, runtime_available
    ...
    if runtime_available([engine]):
        ...
        return True
    print(f"  🔴 runtime conteneur — `{engine}` absent → `forgemaster deploy` échouerait ; installe podman "
          "(via `provision-ct.sh`) ou configure FORGEMASTER_COMPOSE_CMD=docker compose")
    return False
...
def _report_compose_provider(settings: Settings) -> bool:
    ...
    from forgemaster.runtime.backend import compose_engine, compose_provider_available, runtime_available
    ...
    print(f"  🔴 provider compose — moteur présent mais compose `{cmd[0]}` absent "
          "(`apt install podman-compose`, via `provision-ct.sh`) → `forgemaster deploy` échouerait au build")
    return False
...
def _report_runner(settings: Settings) -> bool:
    ...
    **Second consommateur depuis le 2026-08-07** : la MAJ s'en sert pour refuser un wheel dont l'interface
    ...
    if runner.is_file():
        ...
        return True
    print(f"  🔴 runner verify — absent ({runner}) → `forgemaster gate verify` fail-close ; "
          "sème-le (`provision-ct.sh`) ou configure FORGEMASTER_VERIFY_RUNNER")
    return False
...
def _report_tun() -> bool:
    ...
    `forgemaster deploy`) meurt (`failed to read from slirp4netns sync pipe: EOF`) → tout deploy browser-game
    échoue AU BUILD. TUN n'est PAS une feature LXC → device-passthrough (cf. docs/install.md)."""
    if _tun_present():
        print("  ✅ device TUN — `/dev/net/tun` présent (build réseau de `forgemaster deploy` OK)")
        return True
    print("  🔴 device TUN — `/dev/net/tun` absent → le build d'image `forgemaster deploy` échouera "
...
def _report_mcp(settings: Settings) -> bool:
    ...
    if not st["configured"]:
        ...
        return True
    if not st["healthy"]:
        print(f"  🔴 MCP — {st['reason']} → rafraîchis (`forgemaster mcp wire <projet>` ou re-dispatch)")
        for s in st["stale"]:
