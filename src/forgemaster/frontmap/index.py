...
Jumeau strict de `forgemaster.codemap.index` (mêmes invariants : SoT bare + worktrees éphémères → aucun
checkout
...
# marqueur PROPRE au forgemaster (découplé du layout front-map).
_BUILT_MARKER = ".forgemaster-frontmap-built"
...
def frontmap_argv(*args: str) -> list[str]:
    ...
    échouerait. `-m frontmap` utilise le venv du forgemaster (où front-map est installé), sans dépendance
    PATH."""
...
def ensure_index(
    ...
    Cache hit si le dossier `(sha, version)` courant porte déjà le marqueur `.forgemaster-frontmap-built`
    (rien
    ...
    marker.write_text(version, encoding="utf-8")  # marqueur forgemaster APRÈS build OK → cache-hit ultérieur
    return IndexHandle(project=project, ref=ref, sha=sha, root=root)
