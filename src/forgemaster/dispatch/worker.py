...
from forgemaster.dispatch import abort, jobs, reconcile, worktree
...
from forgemaster.provision import facet as facet_mod
from forgemaster.provision.mcp import inject_mcp_config
from forgemaster.roadmap import model, resolver
from forgemaster.roadmap.prompt import build_fix_prompt, build_worker_prompt, findings_block
...
def _make_default_runner(out_path: str, conn: sqlite3.Connection | None = None,
    ...
    thread-locale `conn` : c'est le handle explicite que `forgemaster abort` lit en DB (fin du `pkill`
    fragile).
...
def dispatch_next(conn: sqlite3.Connection, settings: Settings, *, feature_ref: str,
    ...
    réussit** (le primitive est complet — un `forgemaster dispatch` CLI seul avance sa task), révoque
    ...
    # SURFACE `needs_terminal` → l'humain la mène via `forgemaster interview <projet>` (terminal interactif).
    ...
    def _on_success(parsed: dict) -> None:
        ...
        # Clôt la task `in_progress`→`done` : la transition appartient au PRIMITIVE (un `forgemaster dispatch`
        # CLI seul complète sa task, sans dépendre de la boucle `forgemaster run`). Sans ça, un dispatch CLI
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster dispatch <feature>` : résout la NEXT task, refuse si aucune (gate anti-dispatch),
    ...
    if report.get("needs_terminal"):
        ...
        print(f"🖐 {args.feature}/{report['task']} — interview terminale requise (task interactive) : "
              f"lance `forgemaster interview {project}` dans un terminal.")
