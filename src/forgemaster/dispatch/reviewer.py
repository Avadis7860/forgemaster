...
idempotent, best-effort) dans le forgemaster lightweight — la keystone que le pivot n'avait pas re-portée. Le
...
from forgemaster.dispatch import jobs, worker
...
def _journal_non_run(conn: sqlite3.Connection, feature_ref: str, reason: str, *,
    ...
    **Best-effort, non négociable** : une trace ratée ne masque ni ne casse le résultat métier → `try/except`
    ...
    try:
        ...
        logging.getLogger("forgemaster").warning("non_run non journalisé (%s) : %s", feature_ref, exc)
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster gate review-dispatch <feature>` : dispatche le reviewer Tier-1 (si prêt) et écrit le
