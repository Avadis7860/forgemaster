...
- **CLI cross-process** — `forgemaster abort <projet>` (autre process) lit les pgids en DB, les tue ;
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster abort <project> [--feature <slug>]` : arrête proprement le run en cours (workers
    tués
    ...
    if result["aborted"]:
        print(f"abort {scope} : {result['aborted']} worker(s) arrêté(s) — mutex/worktree libéré(s), rien "
              f"mergé (fail-closed). Relançable : `forgemaster run {args.project}`.")
    else:
