"""orchestrator — boucle de dispatch parallèle sur la roadmap d'un projet (`forgemaster run`). Enchaîne le
...
branches feature ; `forgemaster merge --go` reste un gate humain séparé).
"""
...
from forgemaster.dispatch import abort, reviewer, worker
from forgemaster.dispatch import woaw as woaw_dispatch
from forgemaster.dispatch import worktree as worktree_mod
from forgemaster.gate import merge as merge_gate
from forgemaster.gate import toolchain, verify
...
from forgemaster.tools import tools_env
...
def _dispatch_one(settings: Settings, project: str, feature: str,
    ...
    try:
        ...
        try:
            ...
            # re-drain (alerte actionnable + `forgemaster redrain <p>/<f>`).
...
def run_project(conn: sqlite3.Connection, settings: Settings, *, project: str,
    ...
    # lui-même reste dispatchable (sa task interactive est tenue pour `forgemaster interview`). C'est
    ...
    with ThreadPoolExecutor(max_workers=max_parallel) as pool:
        while True:
            # Abort humain (sentinelle posée par `abort.request_abort` : bouton UI, `forgemaster abort`,
            # Ctrl-C) :
            ...
            for fut in done_set:
                ...
                if report.get("needs_terminal"):
                    needs_interview.add(slug)           # tenue pour `forgemaster interview` — pas un échec
                elif report.get("rate_limited"):
...
def _summarize(project: str, reports: list[dict], failed: set[str],
    ...
    `needs_interview` (v12) = features dont la next task est `interactive` : tenues pour
    `forgemaster interview` (surfacées, pas comptées en échec). `held_for_socle` = features de travail
    NON dispatchées parce que le socle du projet n'est pas encore mergé dans `dev`. `rate_limited` (v15)
    = features tenues par le plafond 5 h de l'org (rejet `claude -p`) : ni ok ni échec, re-dispatchables
    après reset. `interrupted` (v16) =
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster run <project> [--max-parallel N]` : draine la roadmap en parallèle, imprime le
    rapport
    ...
    if summary.get("aborted"):
        print(f"run {args.project} : INTERROMPU (abort humain) — workers arrêtés, mutex/worktree libéré(s), "
              f"rien mergé (fail-closed). Relançable : `forgemaster run {args.project}`.")
        return 130
    c = summary["counts"]
    ...
    for feat in summary.get("needs_interview", []):
        print(f"  🖐 {feat} — interview terminale requise (task interactive, non dispatchable en headless) : "
              f"lance `forgemaster interview {args.project}` dans un terminal.")
    ...
    if held_for_socle:
        ...
        print(f"    → merge d'abord le socle (GO humain) : `forgemaster merge {args.project}/<socle> --go`, "
              f"puis relance `forgemaster run {args.project}`.")
    ...
    if rate_limited:
        ...
        print(f"    → relance après le reset du plafond : `forgemaster run {args.project}` "
    ...
    if interrupted:
        ...
        print(f"    → relance : `forgemaster run {args.project}` (les tasks sont revenues `todo`).")
    for fin in summary["finalizations"]:
        if fin["merge_ready"]:
            print(f"  ✅ {fin['feature']} — MERGE-READY (Tier-0 + review OK) → `forgemaster merge "
            f"{args.project}"
                  f"/{fin['feature']} --go`")
        else:
