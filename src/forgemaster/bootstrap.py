...
manifeste (`bootstrap.yaml` sous FORGEMASTER_HOME), au 1er démarrage, via le primitif d'adoption (P1
...
- **Donnée, pas secret** : le manifeste ne porte que des `credential_ref` OPAQUES (repo privé) ou rien
  (repo public). Le token se lie au wizard (`/setup`) ou via `forgemaster bootstrap --token-file` → store ;
...
from forgemaster.secrets import build_store, cred_resolver
...
# `forgemaster bootstrap --token-file <fichier>` — jamais dans ce fichier ni dans un repo.
tools:
  - slug: forgemaster
    source_url: https://github.com/OWNER/forgemaster.git
  # - slug: code-map
...
def manifest_path(settings: Settings) -> Path:
    """Chemin du manifeste d'outils : `<FORGEMASTER_HOME>/bootstrap.yaml`. Déterministe, sous config."""
...
def write_template(settings: Settings) -> Path:
    """Écrit un manifeste gabarit commenté sous FORGEMASTER_HOME (`forgemaster bootstrap --init`). Ne JAMAIS
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster bootstrap` : `--init` écrit un gabarit ; sinon lit le manifeste et adopte chaque
    ...
    if getattr(args, "init", False):
        try:
            ...
            print(f"erreur : {exc}")
            return 1
        print(f"manifeste gabarit écrit → {path} (édite-le puis relance `forgemaster bootstrap`)")
        return 0
    try:
        manifest = load_manifest(settings)
    ...
    if manifest is None:
        print(f"aucun manifeste ({manifest_path(settings)}) — rien à amorcer "
              "(`forgemaster bootstrap --init` pour un gabarit).")
        return 0
