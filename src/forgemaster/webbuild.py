"""webbuild — build de la SPA (Vite) depuis un checkout source. Réutilisé par `forgemaster setup` (chemin
...
dist voyage dans le wheel (buildée au packaging, chez le mainteneur/CI où Node est présent).
`forgemaster setup` couvre le cas où l'on travaille **depuis les sources** : builder le front sur place
quand Node est là. On ne re-committe JAMAIS la dist dans git (respecte `docs/specs/web-forgemaster-spa.md`).
"""
...
def build_front(web_dir: Path, *, clean_install: bool = True) -> Path:
    ...
    if not npm:
        raise FrontBuildError(
            ...
            "  • installe Node (https://nodejs.org) puis relance `forgemaster setup`, ou\n"
...
def find_codemap_src(start: Path | None = None) -> Path | None:
    ...
    depuis le checkout forgemaster. `None` si absent. Sert au chemin from-clone : en install wheel code-map
    est
...
def _install_from_sibling(module: str, src: Path) -> str | None:
    ...
    reconstruit un wheel éditable à chaque appel, et `forgemaster setup` le paierait × 4 pour rien).
    """
...
def ensure_codemap() -> str:
    ...
    (`src/forgemaster/codemap/index.py` invoque `sys.executable -m codemap`). En install **wheel**, code-map
    est
    déjà empaqueté et aucun sibling n'existe (rien à faire). En **from-clone**, code-map n'est PAS dans les
    sources forgemaster : on l'installe **en éditable** depuis le checkout sibling `../code-map`. **Jamais
    ...
    if src is None:
        ...
        return ("code-map introuvable → l'onglet Flow restera indisponible. Clone-le en sibling du "
        "forgemaster "
                "(`git clone …/code-map` à côté) puis relance `forgemaster setup`, ou installe le wheel "
                "packagé "
...
# au contrat (`bundles/base/CLAUDE.md`) : le forgemaster étant lui-même un projet, ses sessions/worktrees
# doivent
# pouvoir les interroger. En install wheel les CLIs viennent du provisioning (`tools.MAP_REPOS`) ; ce câblage
# sert le chemin **from-clone** (dev) depuis les siblings. Trou historique : `forgemaster setup` ne câblait
# QUE
...
def find_map_src(repo: str, module: str, start: Path | None = None) -> Path | None:
    ...
    checkout forgemaster. `None` si absent."""
    here = (start or Path(__file__)).resolve()
    for base in here.parents:
...
def ensure_map(repo: str, module: str) -> str:
    ...
    **Jamais fatal** — une carte absente dégrade l'anti-archéologie, pas le démarrage ; message actionnable.
    ...
    if src is None:
        ...
        return (f"{module} introuvable → clone {repo} en sibling du forgemaster puis relance `forgemaster "
        f"setup`, "
...
def ensure_maps() -> list[str]:
    """Câble les **4 cartes** du framework dans le venv du checkout forgemaster (from-clone) : code-map (Flow)
    +
