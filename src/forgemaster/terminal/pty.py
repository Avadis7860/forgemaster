...
from forgemaster.core import fs
from forgemaster.terminal.registry import PtySession, PtySessionRegistry
...
def interview_argv(project: str, shell: str = "/bin/bash") -> list[str]:
    ...
    shell partagé). PUR. `bash -lc 'exec forgemaster interview <project>'` : le `-l` charge le PATH
    (forgemaster via
    `/etc/profile.d/forgemaster-path.sh`, cf. `shell_env`), puis `exec` **remplace** le shell par l'interview
    →
    aucun prompt où une frappe pourrait se ré-router, et à la sortie de l'interview le PTY reçoit l'EOF → la
    session se ferme d'elle-même. `forgemaster interview` porte tout le cycle (worktree reserve → `claude`
    interactif TTY-hérité → reconcile) et regate l'auth Claude côté CLI (affichée dans le PTY)."""
    return [shell, "-lc", f"exec forgemaster interview {shlex.quote(project)}"]
...
def shell_env() -> dict[str, str]:
    ...
    injecté. Le toolchain (`forgemaster` + maps + `node`) est ré-ajouté APRÈS ce reset par
    `/etc/profile.d/forgemaster-path.sh` que `deploy/provision-ct.sh` installe (le bon étage : le profil, pas
    le
    daemon)."""
...
def classify_exit(code: int | None) -> str:
    ...
    `bash -lc 'exec forgemaster interview <p>'` : un `exec` échoué (`forgemaster` introuvable / non exécutable
    → PATH
...
async def serve_project_terminal(websocket, registry: PtySessionRegistry, *, session_key: str,
    ...
    n'en fait qu'une bannière). Chaque flavor (shell `bash -l` du projet, interview `forgemaster interview`) a
    sa
...
async def _relay(websocket, session: PtySession, epoch: int, cursor: int) -> str:
    ...
    if exc is not None:                                      # transport cassé côté client → déconnexion
        logging.getLogger("forgemaster").info("relais terminal interrompu (%s) → détache", type(exc).__name__)
        return _DISCONNECT
