...
Le forgemaster dispatche des workers `claude -p` **locaux** : ils s'authentifient via le CLI `claude` de la
...
Modèle : auth **par machine** (le CLI `claude` lit `$HOME`), pas par projet. Le forgemaster n'embarque, ne
...
AUTH_HINT = (
    ...
    "authentifier TON compte. Le forgemaster utilise l'auth officielle du CLI `claude` — jamais un token "
    "partagé ni embarqué."
)
