...
from forgemaster.git.internal import GitOpError, InternalGit, classify_push_error, credential_env
from forgemaster.projects.deployments import ensure_deployments
from forgemaster.provision import BundleError, load_bundle, read_bundle_manifest, validate_bundle
...
# tampon `bundle@version` semé dans le SoT (SoT-and-derive)
_PROVENANCE_PATH = ".forgemaster/provenance.toml"
...
def _render_provenance(bundle: str, version: str, created_at: str,
    ...
    """Le tampon de provenance semé dans le SoT (`.forgemaster/provenance.toml`) : de quel `bundle@version` ce
...
def create_project(conn: sqlite3.Connection, settings: Settings, *,
    ...
    try:
        validate_bundle(project_type)   # fail-closed : type hors registre OU manifeste invalide (avant effet)
    except BundleError as exc:
        # Un type inconnu PEUT être un type ajouté APRÈS le build de ce forgemaster (cap silencieux) : si le
        # miroir SoT local le connaît, enrichir le message (nomme le type + retard + « réinjecte ») au lieu
        # du sec « type inconnu ». Lazy import → évite le cycle registry↔build_provenance ; l'ordre
        # fail-closed est intact (on relève toujours AVANT le moindre effet).
        from forgemaster import build_provenance
        hint = build_provenance.stale_type_hint(settings, project_type)
        if hint:
            raise BundleError(f"{exc} — {hint}") from exc
        raise
    ...
    if not source_url:
        ...
        # import paresseux : lecteur de manifeste
        from forgemaster.provision.derive import template_provenance
...
def _seed_launch_roadmap(conn: sqlite3.Connection, *, slug: str, project_type: str) -> None:
    ...
    **paresseux** de `roadmap.seed` : casse le cycle d'import `registry ↔ roadmap.model` (qui importe
    ...
    try:
        from forgemaster.roadmap.seed import seed_launch_roadmap
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster project <action>` (create|list|get)."""
