...
from forgemaster.projects.deployments import get_deployment, set_deployment
from forgemaster.projects.registry import get_project
from forgemaster.runtime.backend import ComposeBackend, ComposeError, PodmanCompose, is_running
...
def _compose_env(name: str, dep: dict) -> dict[str, str]:
    ...
    chaque appel** et son `ports:` est fail-loud (`${FORGEMASTER_PORT:?}`) → FORGEMASTER_PORT doit être
    présent, sinon
    ...
    return {"COMPOSE_PROJECT_NAME": name, "FORGEMASTER_PORT": str(dep.get("port") or 0)}
...
def deploy(conn: sqlite3.Connection, settings: Settings, *, slug: str, branch: str,
    ...
    port = int(res["port"])
    # Le compose publie `${FORGEMASTER_PORT}` (contrat semé par P3) ; on l'injecte + le nom de projet dans
    # l'env.
    env = {"FORGEMASTER_PORT": str(port), "COMPOSE_PROJECT_NAME": name}
    try:
...
def restart(conn: sqlite3.Connection, settings: Settings, *, slug: str, branch: str,
    ...
    **monté** (après un `down`, ré-`up`). Un échec pose `unhealthy` et lève `ValueError`."""
    ...
    name = compose_project_name(slug, branch)
    workdir = deploy_dir_for(settings, slug, branch)
    # port réservé → FORGEMASTER_PORT du re-parse compose
    dep = get_deployment(conn, project_id, branch)
    try:
...
def teardown_preview(conn: sqlite3.Connection, settings: Settings, *, slug: str, feature: str,
    ...
    if wt.exists():
        ...
        with contextlib.suppress(ComposeError):
            backend.down(name, wt, env={"COMPOSE_PROJECT_NAME": name, "FORGEMASTER_PORT": "0"})
    ports.release(conn, project=slug, purpose=_preview_purpose(feature))
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster deploy <up|down|restart|status> <slug> <branch>`. Corps canonique (open_db, branch
    sur
