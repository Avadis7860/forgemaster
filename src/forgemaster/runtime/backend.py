...
# daemon (`BWS_ACCESS_TOKEN`, `FORGEMASTER_*`, `GITHUB_TOKEN`…) : l'env de build/run d'un service est scopé
# par
# construction — il ne reçoit que cette base ⊕ l'overlay explicite de l'engine (`FORGEMASTER_PORT`,
...
class PodmanCompose:
    ...
    def _compose(self, project_name: str, workdir: Path, *args: str,
        ...
        """Exécute `<cmd> -p <name> <args>` dans `workdir`. L'overlay (`FORGEMASTER_PORT`,
        `COMPOSE_PROJECT_NAME`)
        est requis pour l'interpolation `${VAR}` : le compose est **re-parsé à CHAQUE sous-commande** (up,
        down, restart, logs…) et son `ports:` est fail-loud (`${FORGEMASTER_PORT:?}`) → l'engine injecte
        toujours
