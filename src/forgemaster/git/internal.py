...
- **#7/#12** : worktree **attaché au SoT partagé** (`worktree add -B <branch> <path> <base>`, jamais
  `checkout -B <base>`), création **sérialisée par `flock`** sur `<sot>/forgemaster-worktree.lock` (le git-dir
...
_LOCK_NAME = "forgemaster-worktree.lock"
...
def _worktree_lock(sot: str | Path) -> Iterator[None]:
    """Sérialise les mutations de worktree par `flock` sur `<sot>/forgemaster-worktree.lock` (le git-dir du
...
class InternalGit:
    ...
    push authentifié par l'ambiant (compat historique). Ce paquet n'importe JAMAIS `forgemaster.secrets` : la
    ...
    def _seed_base(self, sot: Path, payload: Mapping[str, str] | None = None) -> None:
        ...
        if _git(sot, "show-ref", "--verify", "--quiet", "refs/heads/dev").ok:
            return
        env = writeback_env(("forgemaster", "forgemaster@localhost"))
        tree_sha = self._seed_tree_with_payload(sot, dict(payload or {}), env)
        commit = run.run(
            ["git", "-C", str(sot), "commit-tree", tree_sha, "-m", "root: forgemaster seed"], env=env)
        if not commit.ok:
