...
(`.codemap/calls.manifest.json`) mais un **marqueur propre au forgemaster** (`.forgemaster-index-built`) écrit
APRÈS
...
# marqueur PROPRE au forgemaster (découplé du layout interne code-map).
_BUILT_MARKER = ".forgemaster-index-built"
...
def codemap_argv(*args: str) -> list[str]:
    ...
    échouerait. `-m codemap` utilise le venv du forgemaster (où code-map est installé), sans dépendance
    PATH."""
...
def ensure_index(
    ...
    Cache hit si le dossier `(sha, schema_version)` courant porte déjà le marqueur `.forgemaster-index-built`
    ...
    marker.write_text(schema, encoding="utf-8")  # marqueur forgemaster APRÈS build OK → cache-hit ultérieur
    return IndexHandle(project=project, ref=ref, sha=sha, root=root)
