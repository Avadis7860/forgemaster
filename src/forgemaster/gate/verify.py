...
config (`FORGEMASTER_VERIFY_RUNNER` ou `<home>/runners/render_check.js`), pas un chemin `.claude/` en dur.
...
def runner_path(settings: Settings) -> Path:
    """Chemin du runner Node (Playwright) : `$FORGEMASTER_VERIFY_RUNNER`, sinon
    `<home>/runners/render_check.js`
...
MARKERS_FILE = ".forgemaster/verify-markers.json"
...
def read_verify_contract(workdir: Path) -> dict:
    """Contrat de preuve **déclaré par le worker** dans `<workdir>/.forgemaster/verify-markers.json`. Deux
    formes,
...
def autoverify_feature(conn: sqlite3.Connection, settings: Settings, *, project: str, feature: str,
    ...
    from forgemaster.gate.advertised_authority import check_served_authority
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster gate verify <feature>` : **sans manifeste stdin** (tty/vide) → self-deploying
