...
- **verdict SHA-bound CACHÉ** sous `settings.home` ; `gate/merge.evaluate_gate` le **LIT** seulement. Le
  ...
  `forgemaster gate toolchain` / `POST …/toolchain`) qui **écrit** le verdict.
...
- **DÉCLARATION pour le résidu** (renversement 2026-07-31, cf. `docs/specs/tier0-native-toolchain-gate.md`
  ...
  déclenche le groupe **`declared`**, monté depuis la table `[bundle.gate]` du `.forgemaster/bundle.toml` de
  la
...
from forgemaster.core.run import RunTimeout, run
...
_ABSENT_MSG = {
    ...
    "declared": "source non couverte par une route connue (ni web/, ni node hors web/, ni *.py) et aucune "
                "toolchain déclarée — ajoute une table [bundle.gate] dans .forgemaster/bundle.toml, ex. :\n"
                '  [bundle.gate]\n'
                '  steps = [\n'
...
def detect_groups(worktree: Path) -> list[str]:
    ...
    (`[bundle.gate]` du `.forgemaster/bundle.toml`). Descriptif — l'autorité du RUN est `_steps_for` (qui
    porte
    le fail-closed)."""
    groups: list[str] = []
...
def _declared_steps(worktree: Path) -> list[dict] | None:
    """Steps **déclarés par le projet** dans `[bundle.gate].steps` du `.forgemaster/bundle.toml` de la
    worktree,
    ...
    **Aucun hardcode de langage** : on ne valide QUE la forme, jamais le contenu de l'`argv` (agnosticité par
    ...
    manifest = worktree / ".forgemaster" / "bundle.toml"
...
def cli_dispatch(settings: Settings, args: argparse.Namespace) -> int:
    """Route `forgemaster gate toolchain <feature>` : lance la toolchain (front/backend selon le diff) dans le
    ...
    from forgemaster.roadmap.model import resolve_feature
