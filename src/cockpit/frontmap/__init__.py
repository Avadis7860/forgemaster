...
Frontière : le cockpit ne calcule rien lui-même — il matérialise (`index.ensure_index`, cache SHA) puis
relaie le contrat JSON de `frontmap tokens|primitives|routes` (`catalog.tokens` / `.primitives` / `.routes`).
front-map reste un outil-carte réutilisable hors cockpit ; on ne dépend jamais de son API Python interne
(jumeau strict de `cockpit.codemap`, adapté au fait que front-map négocie sa version via `--version`, pas
