...
2. **Token par-instance** — barrière anti-client-non-navigateur + defense-in-depth. Exigé au handshake via
   le sous-protocole `cockpit.token.<valeur>` (hors des access-logs, cf. `ws_token`). Comparé en temps
...
#: Sigil du token dans `Sec-WebSocket-Protocol` : le client offre `cockpit.token.<valeur>`.
TOKEN_SUBPROTOCOL_PREFIX = "cockpit.token."
...
def match_token_subprotocol(offered: Iterable[str], expected: str) -> str | None:
    """Cherche parmi les sous-protocoles offerts un `cockpit.token.<valeur>` dont la valeur == `expected`
