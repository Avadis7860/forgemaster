...
La base du cockpit monte en **forward-only** (aucune down-migration n'est écrite) : le retour arrière d'une
...
ENTRIES: tuple[tuple[str, str, int], ...] = (
    ("cockpit.db", "vacuum-into", 0o644),   # l'état : projets, features, tasks, jobs
    ("cockpit.env", "copy", 0o600),         # le réglage de l'instance (bind, coffre, câblage MCP)
...
def _build_identity() -> dict[str, str | None]:
    """Version + SHA de build du cockpit qui a PRIS l'instantané. `sha=None` en checkout éditable est un
    état honnête (le wheel n'a pas de tampon), pas une erreur — `read_stamp` ne lève jamais."""
    from cockpit import __version__
    from cockpit.build_provenance import read_stamp
    stamp = read_stamp()
...
def _read_manifest(path: Path) -> dict:
    ...
    return {"valid": True, "created_at": data.get("created_at"), "cockpit": data.get("cockpit"),
