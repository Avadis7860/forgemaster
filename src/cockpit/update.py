"""update — le verbe `cockpit update apply` : poser un wheel LOCAL et revenir tout seul si ça ne sert pas.
...
Le travail réel est fait par `apply_update.py`, script autonome (stdlib pure, zéro import `cockpit`) que ce
...
- **pas d'unité systemd** → la bascule exige un service gérable. On ne va pas inventer une façon de
  redémarrer le cockpit de quelqu'un ;
...
from cockpit.service import stable_link, unit_path
...
def preflight(settings: Settings, *, wheel: str, unit: str | None, scope: str) -> dict:
    ...
    if scope == "system" and os.geteuid() != 0:
        raise UpdateRefused("portée système demandée sans être root — `systemctl` échouerait en plein "
                            ...
                            "utilisateur (`cockpit install-service`).")
    ...
    if not up.is_file():
        raise UpdateRefused(f"aucune unité systemd à {up} — la bascule bleu/vert exige un service gérable. "
                            f"Installe-le : `cockpit install-service`.")
    ...
    if not link.is_symlink():
        raise UpdateRefused(f"{link} n'est pas un lien stable vers le venv actif. Relance "
                            f"`cockpit install-service` (il le pose), puis `systemctl daemon-reload`.")
    if not exec_bin.startswith(str(link) + os.sep):
        raise UpdateRefused(
            ...
            f"le service. Migre-la : `cockpit install-service` puis `systemctl daemon-reload`.")
...
def parse_exec_start(unit_text: str) -> tuple[str, str, int]:
    ...
    du service — le déduire de `cockpit.env` ou d'un défaut sonderait une autre instance que celle qu'on
    ...
    if port is None or not port.isdigit():
        raise UpdateRefused(f"impossible de lire le port dans `ExecStart={' '.join(argv)}` — sans lui, "
                            ...
                            f"automatique. Réinstalle l'unité : `cockpit install-service --port <n>`.")
