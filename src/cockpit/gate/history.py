...
-fallback » de `dispatch-history-persistence`, parce qu'ici le disque **n'est pas volatil** (cockpit local).
...
def prune_verdicts(conn: sqlite3.Connection, project: str, feature: str, *,
    ...
    try:
        ...
        logging.getLogger("cockpit").warning("historique non borné (%s/%s) : %s", project, feature, exc)
